...
async def _handle_b2c_booking_payment(entity: dict) -> None:
    ...
    # Generate PDF invoice — wrapped so a PDF-build failure never blocks the
    # booking confirmation message itself (best-effort degrade to no-invoice).
    ...
    try:
        ...
        logger.error("Booking invoice generation failed for %s: %s", booking_id, e, exc_info=True)
    ...
    slot_time_raw   = booking_data.get("slot_datetime", "")
    slot_time       = (
        slot_time_raw.astimezone(IST).strftime("%a %d %b, %I:%M %p")
        if hasattr(slot_time_raw, "astimezone") else str(slot_time_raw)
    )
