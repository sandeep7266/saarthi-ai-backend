...
def _build_booking_pdf(
    ...
    items_data = [
        ...
        [Paragraph(booking.get("service_name", "—"), cell_s),
         ...
         Paragraph(f"₹{service_price:,.0f}",           num_s)],
    ]
    ...
    # Summary
    summary_data = [
        [Paragraph("Subtotal:",     lbl_s), Paragraph(f"₹{service_price:,.0f}",  amt_s)],
        [Paragraph("Deposit Paid:", lbl_s), Paragraph(f"- ₹{deposit_amount:,.0f}", amt_s)],
        [Paragraph("Balance Due:",  tot_l), Paragraph(f"₹{balance_due:,.0f}",    tot_a)],
    ]
...
def _build_subscription_pdf(
    ...
    items_tbl = Table([
        [Paragraph("Description", hdr_s), Paragraph("Plan",   hdr_s),
         Paragraph("Period",      hdr_s), Paragraph("Amount (₹)", hdr_s)],
        [Paragraph("Saarthi-AI Subscription", td_s), Paragraph(plan.title(), td_s),
         Paragraph(cycle.title(), td_s), Paragraph(f"₹{base_amt:,.2f}", nr_s)],
        [Paragraph("GST @ 18%",   td_s), Paragraph("", td_s),
         Paragraph("",            td_s), Paragraph(f"₹{gst_amt:,.2f}", nr_s)],
    ...
    tot_tbl = Table([[
        ...
        Paragraph(f"₹{amount_inr:,.2f}", tot_l),
